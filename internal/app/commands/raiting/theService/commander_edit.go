...
func (c *RatingTheServiceCommander) Edit(ctx context.Context, inputMsg *tgbotapi.Message) {
	...
	if args == "" {
		c.sendError(ctx, helpEdit, inputMsg.Chat.ID)
		return
	}
	...
	if err != nil {
		c.sendError(ctx, fmt.Sprintf("error idx value: %s", parameters[0]), inputMsg.Chat.ID)
		return
	}
	...
	if err != nil {
		c.sendError(ctx, fmt.Sprintf("error Value value: %s", parameters[2]), inputMsg.Chat.ID)
		return
	}
	service.ReviewsCount, err = strconv.Atoi(parameters[3])
	if err != nil {
		c.sendError(ctx, fmt.Sprintf("error ReviewsCount value: %s", parameters[3]), inputMsg.Chat.ID)
		return
	}
	...
	if err != nil {
		c.sendError(ctx, fmt.Sprintf("fail to get service with idx %d: %v", idx, err), inputMsg.Chat.ID)
		return
	}
